...
def synthesize_cyberlurch_yearly_json(system_prompt: str, user_prompt: str) -> str:
    """Make one edition-level Cyberlurch Yearly provider operation."""
    client = _get_client()
    response = _openai_chat_create(
        client,
        stage="cyberlurch_yearly_synthesis",
        model=OPENAI_MODEL_CYBERLURCH_OVERVIEW,
        messages=[
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": user_prompt},
