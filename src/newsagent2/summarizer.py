...
def synthesize_cyberlurch_monthly_json(system_prompt: str, user_prompt: str) -> str:
    ...
    synthesize_cyberlurch_monthly_json.last_usage = {
        "input_tokens": next((int(getattr(usage, name)) for name in ("prompt_tokens", "input_tokens") if getattr(usage, name, None) is not None), None),
        "output_tokens": next((int(getattr(usage, name)) for name in ("completion_tokens", "output_tokens") if getattr(usage, name, None) is not None), None),
    }
